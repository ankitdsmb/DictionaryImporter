    ...
    ILogger<AiEnhancementStep> logger)
{
    ...
    private readonly bool _enabled =
        configuration.GetValue($"{Root}:EnablePipelineSteps:AiEnhancement", false);

    private readonly int _take =
        configuration.GetValue($"{Root}:AiEnhancement:Take", 50);

    private readonly string _provider =
        configuration.GetValue($"{Root}:AiEnhancement:Provider", "Ollama");

    private readonly string _model =
        configuration.GetValue($"{Root}:AiEnhancement:Model", "llama3.1");

    private readonly int _maxTokens =
        configuration.GetValue($"{Root}:AiEnhancement:MaxTokens", 250);

    private readonly double _temperature =
        configuration.GetValue($"{Root}:AiEnhancement:Temperature", 0.2);

    private readonly int _parallelCalls =
        configuration.GetValue($"{Root}:AiEnhancement:ParallelCalls", 1);

    private readonly int _bulkBatchSize =
        configuration.GetValue($"{Root}:AiEnhancement:BulkBatchSize", 1);

    private readonly AiExecutionMode _mode =
        Enum.TryParse(configuration.GetValue($"{Root}:AiEnhancement:Mode", "SingleBest"),
            ignoreCase: true,
