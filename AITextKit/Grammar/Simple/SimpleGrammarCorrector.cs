using DictionaryImporter.AITextKit.Grammar.Infrastructure.Engines;

namespace DictionaryImporter.AITextKit.Grammar.Simple;
...
public sealed class SimpleGrammarCorrector : IGrammarCorrector, IDisposable
{
        ...
        ILoggerFactory loggerFactory)
    {
        ...
        _settings = new EnhancedGrammarConfiguration
        {
            MinDefinitionLength = configuration?.GetValue("Grammar:MinDefinitionLength", 20) ?? 20,
            DefaultLanguage = configuration?.GetValue("Grammar:DefaultLanguage", "en-US") ?? "en-US",
