...
namespace DictionaryImporter.Infrastructure.Parsing.SynonymExtractor
{
    public sealed class EnglishChineseSynonymExtractor : ISynonymExtractor
    {
        ...
        private readonly ILogger<EnglishChineseSynonymExtractor> _logger;

        // STRICT PATTERNS - only capture high-confidence synonyms
        private static readonly Regex[] HighConfidencePatterns =
        {
            // Pattern 1: "X means Y"
            new Regex(@"^\s*(?<word1>\w+)\s+means\s+(?<word2>\w+)\s*$",
                RegexOptions.IgnoreCase | RegexOptions.Compiled),
            
            // Pattern 2: "X is Y"
            new Regex(@"^\s*(?<word1>\w+)\s+is\s+(?<word2>\w+)\s*$",
                RegexOptions.IgnoreCase | RegexOptions.Compiled),
                
            // Pattern 3: "X is the same as Y"
            new Regex(@"^\s*(?<word1>\w+)\s+is\s+the\s+same\s+as\s+(?<word2>\w+)\s*$",
                RegexOptions.IgnoreCase | RegexOptions.Compiled),
                
            // Pattern 4: "X and Y are synonyms"
            new Regex(@"^\s*(?<word1>\w+)\s+and\s+(?<word2>\w+)\s+are\s+synonyms\s*$",
                RegexOptions.IgnoreCase | RegexOptions.Compiled),
        };
        ...
        {
            // "X, or Y, ..."
            new Regex(@"^\s*(?<word1>\w+)\s*,\s*or\s+(?<word2>\w+)\s*,",
                RegexOptions.IgnoreCase | RegexOptions.Compiled),
                
            // "X (also Y)"
            new Regex(@"^\s*(?<word1>\w+)\s*\(\s*also\s+(?<word2>\w+)\s*\)",
                RegexOptions.IgnoreCase | RegexOptions.Compiled),
        };
        ...
        private static readonly Regex[] LowConfidencePatterns =
        {
            ...
            new Regex(@"synonymous\s+with\s+(?<word2>\w+)",
                RegexOptions.IgnoreCase | RegexOptions.Compiled),
                
            // "similar to Y"
            new Regex(@"similar\s+to\s+(?<word2>\w+)",
                RegexOptions.IgnoreCase | RegexOptions.Compiled),
        };
        ...
        private static readonly Regex[] DefinitionReplacementPatterns =
        {
            ...
            new Regex(@"^If you (?<word1>\w+) [^,]+, you (?<word2>\w+) (?:it|something|one)\.?$",
                RegexOptions.IgnoreCase | RegexOptions.Compiled),
                
            // "To X means to Y"
            new Regex(@"^To (?<word1>\w+) means to (?<word2>\w+)(?:\.|$)",
                RegexOptions.IgnoreCase | RegexOptions.Compiled),
                
            // "Xing is Ying"
            new Regex(@"^(?<word1>\w+ing) is (?<word2>\w+ing)(?:\.|$)",
                RegexOptions.IgnoreCase | RegexOptions.Compiled),
        };

        public EnglishChineseSynonymExtractor(ILogger<EnglishChineseSynonymExtractor> logger)
        {
            _logger = logger;
        }
