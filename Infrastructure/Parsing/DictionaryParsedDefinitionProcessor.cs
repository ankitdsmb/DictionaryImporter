...
public sealed class DictionaryParsedDefinitionProcessor : IParsedDefinitionProcessor
{
    ...
    private async Task ProcessBatchAsync(string sourceCode, CancellationToken ct)
    {
        ...
        await foreach (var entry in StreamEntriesAsync(sourceCode, ct))
        {
            ...
            var parsedDef = ParseEntry(entry, parser, sourceCode);
    ...
    private async IAsyncEnumerable<DictionaryEntry> StreamEntriesAsync(string sourceCode, [EnumeratorCancellation] CancellationToken ct)
    {
    ...
    private async Task WriteExamples(long parsedId, ParsedDefinition parsed, IExampleExtractor extractor, string sourceCode, CancellationToken ct)
    {
    ...
    private async Task WriteSynonyms(long parsedId, DictionaryEntry entry, ParsedDefinition parsed, ISynonymExtractor extractor, string sourceCode, CancellationToken ct)
    {
    ...
    private async Task WriteEtymology(long dictionaryEntryId, DictionaryEntry entry, ParsedDefinition parsed, IEtymologyExtractor extractor, string sourceCode, CancellationToken ct)
    {
    ...
    private async Task WriteCrossReferences(long parsedId, ParsedDefinition parsed, string sourceCode, CancellationToken ct)
    {
    ...
    private async Task WriteAlias(long parsedId, DictionaryEntry entry, ParsedDefinition parsed, string sourceCode, CancellationToken ct)
    {
    ...
    private IEnumerable<ParsedDefinition> ParseEntry(DictionaryEntry entry, IDictionaryDefinitionParser parser, string sourceCode) => parser.Parse(entry) ?? new[] { CreateFallback(entry, sourceCode) };

    private async Task<(string ProcessedText, long? NonEnglishTextId)> ProcessText(string text, string fieldType, string sourceCode, CancellationToken ct)
    {
