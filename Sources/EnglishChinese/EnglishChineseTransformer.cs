...
namespace DictionaryImporter.Sources.EnglishChinese
{
    public sealed class EnglishChineseTransformer : IDataTransformer<EnglishChineseRawEntry>
    {
        ...
        {
            if (!SourceDataHelper.ShouldContinueProcessing(SourceCode, _logger)) yield break;
        ...
        private IEnumerable<DictionaryEntry> ProcessEnglishChineseEntry(EnglishChineseRawEntry raw)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(definition))
                {
                    ...
                    if (!IsAbbreviation(raw.Headword) && !SimpleEngChnExtractor.ContainsChinese(definition))
                    {
                        _logger.LogWarning(
                            "ENG_CHN entry '{Word}' may have lost Chinese characters. Raw: {RawPreview}",
                            raw.Headword, GetPreview(raw.RawLine, 100));
                    }
                }
                else
                {
                    _logger.LogWarning(
                        ...
                        raw.Headword, GetPreview(raw.RawLine, 50));
                }
        ...
        private static bool IsAbbreviation(string word)
        {
            ...
            if (Regex.IsMatch(cleanWord, @"\d+[A-Za-z]|[A-Za-z]\d+")) return true;
            ...
            if (!string.IsNullOrEmpty(lettersOnly))
            {
                ...
                if (uppercaseRatio > 0.7 && cleanWord.Length <= 8) return true;
            }

            // 4. Contains dots between letters (like "U.S.A.", "a.m.", "p.m.")
            if (word.Contains('.') && Regex.IsMatch(word, @"[A-Za-z]\.[A-Za-z]")) return true;
            ...
            if (word.Length <= 3)
            {
                ...
                if (letterCount > 0)
                {
                    ...
                    if (uppercaseCount >= letterCount) return true;
                }
            }
