...
namespace DictionaryImporter.Sources.Common.Helper
{
    ...
    public static class CollinsSourceDataHelper
    {
        ...
        public static bool TryParseSenseHeader(string line, out CollinsSenseRaw sense)
        {
            ...
            // Strict guard against continuation text
        ...
        public static bool IsDefinitionContinuation(string line, string currentDefinition)
        {
            ...
            // 3. Starts with continuation punctuation
            if (trimmed.StartsWith("(") || trimmed.StartsWith("-") || trimmed.StartsWith("—"))
                return true;

            // 4. Previous definition ends with continuation punctuation
        ...
        #endregion CollinsExtractor Helper Methods
        ...
        public static string CleanExampleText(string example)
        {
            ...
            cleaned = CleanText(cleaned, false, true);
        ...
        #endregion Text Processing and Cleaning
        ...
        public static IReadOnlyList<string> ExtractExamples(string definition)
        {
            ...
            if (exampleIndex >= 0)
            {
                ...
                var end = FindNextMarker(definition, start);
        ...
        #endregion Section Extraction

        #region Cross-Reference Extraction
        ...
        public static IReadOnlyList<CrossReference> ExtractCrossReferences(string definition)
        {
            ...
            foreach (var cr in crossRefs)
            {
                ...
                if (target.Length > 150) continue; // Safety limit
        ...
        private static IReadOnlyList<CrossReference> FastExtractCrossReferencesFromText(string text)
        {
            ...
            // Handle "→see:" with size limits
            if (text.Contains("→see:", StringComparison.OrdinalIgnoreCase))
            {
                var matches = Regex.Matches(
                    text,
                    @"→see:\s*(?<target>[A-Za-z][A-Za-z\s\-']{1,100})",
                    RegexOptions.IgnoreCase | RegexOptions.Compiled);
            ...
            // Handle "See also:" with splitting and limits
            if (text.Contains("See also:", StringComparison.OrdinalIgnoreCase))
            {
                foreach (Match match in SeeAlsoRegex.Matches(text))
                {
                    ...
                    if (targetsText.Length > 500)
                        targetsText = targetsText.Substring(0, 500);
        ...
        private static IReadOnlyList<string> FastParseTargetWords(string targetsText)
        {
            ...
            foreach (var part in parts)
            {
                ...
                // Remove Chinese characters for cross-refs
                trimmed = ChineseCharRegex.Replace(trimmed, "").Trim();

                // Remove punctuation
                trimmed = trimmed.Trim(',', ';', ' ', '，', '；', '.', '。', ':', '：');

                // Cut at any remaining "see" references
                var seeIndex = trimmed.IndexOf("→see:", StringComparison.OrdinalIgnoreCase);
                if (seeIndex >= 0)
                    trimmed = trimmed.Substring(0, seeIndex).Trim();
        ...
        #endregion Cross-Reference Extraction

        #region Synonym Extraction
        ...
        public static IReadOnlyList<string>? ExtractSynonymsFromExamples(IReadOnlyList<string> examples)
        {
            ...
            foreach (var example in examples)
                AddSynonymsFromText(example, synonyms);

            return synonyms.Count > 0 ? new List<string>(synonyms) : null;
        }

        private static void AddSynonymsFromText(string text, HashSet<string> synonyms)
        {
            if (string.IsNullOrWhiteSpace(text)) return;

            var synonymMatch = SynonymPatternRegex.Match(text);
            if (synonymMatch.Success)
                AddSynonym(synonymMatch.Groups["word"].Value, synonyms);

            var parentheticalMatch = ParentheticalSynonymRegex.Match(text);
            if (parentheticalMatch.Success)
                AddSynonym(parentheticalMatch.Groups["word"].Value, synonyms);

            var orMatch = OrSynonymRegex.Match(text);
            if (orMatch.Success)
            {
                AddSynonym(orMatch.Groups["word1"].Value, synonyms);
                AddSynonym(orMatch.Groups["word2"].Value, synonyms);
            }
        }

        private static void AddSynonym(string word, HashSet<string> synonyms)
        {
        ...
        #endregion Synonym Extraction

        #region Raw Fragment Processing
        ...
        public static string ExtractMainDefinitionFromRawFragment(string rawFragment)
        {
            ...
            return CleanDefinitionText(mainDefinition);
        }
        ...
        public static List<string> CleanDefinitionBatch(IEnumerable<string> definitions)
        {
            ...
            foreach (var definition in definitions)
            {
                ...
                foreach (var marker in SectionMarkers)
                {
                    ...
                    if (index >= 0)
                        withoutSections = withoutSections.Substring(0, index);
                }

                results.Add(CleanDefinitionText(withoutSections));
            }
            return results;
        }
        ...
        #endregion Raw Fragment Processing

        #region Universal Text Cleaning Methods
        ...
        public static List<string> ProcessDefinitionBatch(IEnumerable<string> definitions)
        {
            ...
            foreach (var definition in definitions)
            {
                try
                {
                    ...
                    results.Add(IsValidDefinition(cleaned, true) ? cleaned : string.Empty);
                }
                catch
                {
        ...
        #endregion Universal Text Cleaning Methods
        ...
        private static CollinsSenseRaw? TryParseSimpleNumberedLine(string line)
        {
            ...
            // Quick reject: if line has Chinese markers or section markers
            ...
            // Critical fix: If starts with lowercase or punctuation, likely continuation
            if (rest.Length > 0 && !char.IsUpper(rest[0])) return null;

            // Critical fix: If contains cross-reference marker
            if (rest.Contains("→see:", StringComparison.OrdinalIgnoreCase)) return null;

            // Critical fix: If it's clearly a continuation connector
        ...
        private static string FastNormalizePos(string rawPos)
        {
            ...
            // Try without Chinese characters
            ...
            foreach (var prefix in PosPrefixes)
                if (trimmed.StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    // Default to basic POS based on prefix
        ...
        private static string FastCleanDefinitionText(string definition)
        {
            ...
            var cleaned = ExtractDefinitionCore(definition);
            if (string.IsNullOrWhiteSpace(cleaned)) return cleaned;

            cleaned = RemoveStrayElements(cleaned);
            cleaned = CleanText(cleaned, true, true);
            cleaned = EnsureSentenceStructure(cleaned);

            if (!string.IsNullOrEmpty(cleaned))
                cleaned = EnsureCapitalizationAndPunctuation(cleaned);

            return cleaned.Trim();
        }

        private static string ExtractDefinitionCore(string text)
        {
            if (string.IsNullOrWhiteSpace(text)) return text;

            var result = text;
            result = RemoveSectionHeaders(result);
            result = RemoveGrammarCodes(result);
            result = RemoveCrossReferences(result);
            return ExtractActualDefinition(result);
        }

        private static string RemoveSectionHeaders(string text)
        {
            if (string.IsNullOrWhiteSpace(text)) return text;
        ...
        private static string RemoveGrammarCodes(string text)
        {
            if (string.IsNullOrWhiteSpace(text)) return text;
                ...
                result.StartsWith("ADV-") || result.StartsWith("PHR-"))
            {
                ...
                if (endIndex < result.Length)
                    result = result.Substring(endIndex).TrimStart();
                else
                    result = string.Empty;
            }
            return result;
        ...
        private static string RemoveCrossReferences(string text)
        {
            if (string.IsNullOrWhiteSpace(text)) return text;
        ...
        private static string ExtractActualDefinition(string text)
        {
            if (string.IsNullOrWhiteSpace(text)) return text;
        ...
        private static string RemoveStrayElements(string text)
        {
            if (string.IsNullOrWhiteSpace(text)) return text;
        ...
        private static string EnsureCapitalizationAndPunctuation(string text)
        {
        ...
        private static bool IsValidDefinition(string text, bool lenient = false)
        {
            if (string.IsNullOrWhiteSpace(text)) return false;
