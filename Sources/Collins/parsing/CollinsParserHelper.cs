...
namespace DictionaryImporter.Sources.Collins.Parsing
{
    ...
    public static class CollinsParserHelper
    {
        ...
        public static readonly Regex SeeAlsoRegex =
            new(@"\bSee also:\s*(?<targets>[^.]+)",
                RegexOptions.Compiled | RegexOptions.IgnoreCase);

        public static readonly Regex SeeRegex =
            new(@"\bSee:\s*(?<target>[^.]+)",
                RegexOptions.Compiled | RegexOptions.IgnoreCase);

        public static readonly Regex CrossReferenceRegex =
            new(@"^(?<word>[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)→see:\s*(?<target>[a-z]+)",
                RegexOptions.Compiled | RegexOptions.IgnoreCase);

        // Synonym patterns
        public static readonly Regex SynonymPatternRegex =
            new(@"\b(?:synonymous|synonym|same as|equivalent to|also called)\s+(?:[\w\s]*?\s)?(?<word>[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)",
                RegexOptions.Compiled | RegexOptions.IgnoreCase);

        public static readonly Regex ParentheticalSynonymRegex =
            new(@"\b(?<word>[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s*\((?:also|syn|syn\.|synonym)\)",
                RegexOptions.Compiled | RegexOptions.IgnoreCase);

        public static readonly Regex OrSynonymRegex =
        ...
        public static readonly Regex EnglishTextRegex =
        ...
        public static readonly Regex ExampleBulletRegex =
            new(@"^•\s*(.+)$", RegexOptions.Compiled | RegexOptions.Multiline);

        public static readonly Regex DomainLabelRegex =
        ...
        public static readonly Regex SenseHeaderEnhancedRegex =
            new(@"^(?:(?<number>\d+)\.\s*)?(?<pos>[A-Z][A-Z\-\s;]+)(?:[/\\]\s*[A-Z][A-Z\-\s;]*)?\t[^\x00-\x7F]*\s*(?<definition>.+)",
        ...
        public static readonly Regex GrammarCodeRegex =
            new(@"\b(?:V-ERG|N-COUNT|N-UNCOUNT|ADJ-GRADED|PHR-CONJ-SUBORD|PHR-V|PHR-ADV|PHR-PREP|V-TRANS|V-INTRANS|V-REFL|V-PASS)\b",
                RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        private static readonly Regex LeadingJunkRegex =
            new(@"^[\s、。，；,\.\(\)（）:：…\/\\;]+", RegexOptions.Compiled);

        private static readonly Regex StrayPunctuationPatternsRegex =
            new(@"^(?:DISCOURSE USES \d+|PHR-CONJ-SUBORD|V-ERG\s*[\/\\]|[A-Z]+\s*[\/\\]|\d+\.|[①②③④⑤⑥⑦⑧⑨⑩]|[⓪❶❷❸❹❺❻❼❽❾])\s*",
                RegexOptions.Compiled | RegexOptions.IgnoreCase);

        private static readonly Regex MultiplePunctuationRegex =
            new(@"[;，。；,\.\(\)（）]{2,}", RegexOptions.Compiled);
        ...
        private static readonly HashSet<string> PosPrefixes = new(StringComparer.OrdinalIgnoreCase)
        {
            ...
            "EXCL", "INTERJ", "SUFFIX", "PREFIX", "COMB", "NUM", "AUX", "MODAL", "QUANT"
        };
        ...
        public static string? ExtractCleanGrammar(string? grammarText)
        {
            ...
            var cleaned = FastCleanText(grammarText.Trim(), removeChinese: true, normalizePunctuation: true);
            if (string.IsNullOrWhiteSpace(cleaned))
                return null;
        ...
        public static string ExtractMainDefinition(string definition)
        {
            ...
            int firstIndex = definition.Length;
        ...
        public static IReadOnlyList<string> ExtractExamples(string definition)
        {
            ...
            int exampleIndex = definition.IndexOf(ExampleSeparator, StringComparison.Ordinal);

            if (exampleIndex >= 0)
            {
                int start = exampleIndex + ExampleSeparator.Length;
                int end = FastFindNextMarker(definition, start);
        ...
        {
            int noteIndex = definition.IndexOf(NoteSeparator, StringComparison.Ordinal);
            if (noteIndex >= 0)
            {
                int start = noteIndex + NoteSeparator.Length;
                int end = FastFindNextMarker(definition, start);
        ...
        {
            int domainIndex = definition.IndexOf(DomainSeparator, StringComparison.Ordinal);
            if (domainIndex >= 0)
            {
                int start = domainIndex + DomainSeparator.Length;
                int end = FastFindNextMarker(definition, start);
        ...
        {
            int grammarIndex = definition.IndexOf(GrammarSeparator, StringComparison.Ordinal);
            if (grammarIndex >= 0)
            {
                int start = grammarIndex + GrammarSeparator.Length;
                int end = FastFindNextMarker(definition, start);
        ...
        public static string? ExtractDomainFromRawFragment(string rawFragment)
        {
            ...
            int labelIndex = rawFragment.IndexOf("【语域标签】：", StringComparison.Ordinal);
            if (labelIndex >= 0)
            {
                int start = labelIndex + "【语域标签】：".Length;
                int end = rawFragment.IndexOf("】", start);
                if (end >= 0)
                {
        ...
        public static string? ExtractUsageFromRawFragment(string rawFragment)
        {
            ...
            // Fast string search
            int grammarIndex = rawFragment.IndexOf("【Grammar】", StringComparison.Ordinal);
            if (grammarIndex >= 0)
            {
                int start = grammarIndex + "【Grammar】".Length;
                int end = rawFragment.IndexOf("】", start);
                if (end >= 0)
                {
        ...
        public static string ExtractMainDefinitionFromRawFragment(string rawFragment)
        {
            ...
            int firstIndex = rawFragment.Length;
            foreach (var marker in SectionMarkers)
            {
        ...
        public static List<string> CleanDefinitionBatch(IEnumerable<string> definitions)
        {
            ...
            foreach (var definition in definitions)
            {
                ...
                foreach (var marker in SectionMarkers)
                {
                    int index = withoutSections.IndexOf(marker, StringComparison.Ordinal);
                    if (index >= 0)
                    {
        ...
        private static string FastCleanDefinitionText(string definition)
        {
            ...
            cleaned = FastCleanText(cleaned, removeChinese: true, normalizePunctuation: true);
            ...
            {
                cleaned = FastEnsureCapitalizationAndPunctuation(cleaned);
            }
        ...
        private static CollinsSenseRaw? CreateSenseFromMatch(Match match)
        {
            ...
            if (!posGroup.Success || !definitionGroup.Success)
                return null;

            int senseNumber = numberGroup.Success && int.TryParse(numberGroup.Value, out var parsed)
                ? parsed
                : 1;
            ...
            foreach (var prefix in PosPrefixes)
            {
                if (trimmed.StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (prefix.StartsWith("V-", StringComparison.OrdinalIgnoreCase) || prefix == "V")
                        return "verb";
                    if (prefix.StartsWith("ADJ-", StringComparison.OrdinalIgnoreCase) || prefix == "ADJ")
                        return "adj";
                    if (prefix.StartsWith("ADV-", StringComparison.OrdinalIgnoreCase) || prefix == "ADV")
                        return "adv";
                    if (prefix.StartsWith("PHR-", StringComparison.OrdinalIgnoreCase))
                        return "phrase";
                }
            }
        ...
        private static bool IsNonDefinition(string text)
        {
            return IsSectionHeader(text) ||
                   IsCrossReferenceOnly(text) ||
                   IsGrammarCodeOnly(text);
        }
