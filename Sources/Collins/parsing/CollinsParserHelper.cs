...
public static class CollinsParserHelper
{
    ...
    public static IReadOnlyList<string>? ExtractSynonymsFromExamples(IReadOnlyList<string> examples)
    {
        ...
        foreach (var example in examples) FastAddSynonymsFromText(example, synonyms);
    ...
    private static CollinsSenseRaw? CreateSenseFromMatch(Match match)
    {
        ...
        var senseNumber = numberGroup.Success && int.TryParse(numberGroup.Value, out var parsed)
            ? parsed
            : 1;
    ...
    private static string FastNormalizePos(string rawPos)
    {
        ...
        if (trimmed.Contains("VERB", StringComparison.OrdinalIgnoreCase))
            return "verb";
        if (trimmed.Contains("ADJECTIVE", StringComparison.OrdinalIgnoreCase) ||
            trimmed.Contains("ADJ", StringComparison.OrdinalIgnoreCase))
            return "adj";
        if (trimmed.Contains("ADVERB", StringComparison.OrdinalIgnoreCase) ||
            trimmed.Contains("ADV", StringComparison.OrdinalIgnoreCase))
            return "adv";
