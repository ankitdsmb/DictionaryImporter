...
public class DynamicPOSProvider(IConfiguration configuration, IMemoryCache cache, HttpClient? httpClient = null)
{
    ...
    public async Task<PartOfSpeechConfig> GetPOSConfigAsync(string language = "en", string dictionaryType = "gutenberg")
    {
        ...
        return await cache.GetOrCreateAsync(cacheKey, async entry =>
        {
            ...
            var config = configuration.GetSection($"Dictionary:POS:{language}:{dictionaryType}")
    ...
    private async Task<PartOfSpeechConfig> FetchPOSConfigAsync(string language, string dictionaryType)
    {
        ...
        try
        {
            ...
            var urls = configuration.GetSection($"Dictionary:POS:{language}:Urls")
                .Get<string[]>();
